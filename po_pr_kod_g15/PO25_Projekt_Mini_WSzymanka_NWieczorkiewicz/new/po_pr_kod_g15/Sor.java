...
public class Sor {
    public static void main(String[] args) {
        ...
        while (budzet.ileZostalo() > 0) {
            ...
            long countDoctors = personelList.stream()
                .filter(p -> p.rolaPersonelu == Personel.Rola.LEKARZ).count();
            long countNurses = personelList.stream()
            ...
            for (Pacjent p : new ArrayList<>(hospitalizowani)) {
                ...
                int szansaWypisu = StanPacjenta.szansaNaWypis(p.stan);
                if (roll < szansaZgonu) deceased.add(p);
                else if (roll < szansaZgonu + szansaWypisu) discharged.add(p);
            }
            ...
            if (!eventPending && dayRand.nextInt(100) < 30) {
                eventPending = true;
            ...
            while (!cont) {
                ...
                if (eventPending) System.out.println("7. Obsłuż zdarzenie");
                System.out.print("> ");
        ...
        try {
            Report.saveReport("report.txt", summary);
         
        } catch (IOException e) {
            System.out.println("Błąd podczas zapisu logów lub raportu: " + e.getMessage());
        }
    }
}
